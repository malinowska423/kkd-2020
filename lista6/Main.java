  ...
  public static void main(String[] args) {
    if (args.length != 3) {
      ...
      try {
        ...
        String OUTPUT_FOLDER = "testy/";
        
        switch (args[0]) {
          case "--e" -> {
            //encode
            coder.encode();
            Files.write(Paths.get(OUTPUT_FOLDER + "filtered_low.tga"), tga.getTGABytes(TGAAnalyzer.pixelsToBytes(coder.filteredLow)));
            Files.write(Paths.get(OUTPUT_FOLDER + "filtered_high.tga"), tga.getTGABytes(TGAAnalyzer.pixelsToBytes(coder.filteredHigh)));
            Files.write(Paths.get(OUTPUT_FOLDER + "encoded_low.tga"), tga.getTGABytes(coder.bytes));
            Files.write(Paths.get(OUTPUT_FOLDER + "encoded_high.tga"), tga.getTGABytes(TGAAnalyzer.pixelsToBytes(coder.quantified)));
          ...
          case "--d" -> {
            //decode
            Files.write(Paths.get(OUTPUT_FOLDER + "decoded_low.tga"), coder.decode());
